...
public class ApplicationShortlistService {
    ...
    public List<ShortlistResult> shortlistApplications(Long jobId, int topN) {
        ...
        for (Application app : applications) {
            try {
                // Get or parse CV data
                ParsedCVData cvData = cvParsingService.getParsedCVData(app.getId());
                
                // If CV not parsed yet, parse it now
                if (cvData == null) {
                    logger.info("Parsing CV for application {}", app.getId());
                    cvParsingService.parseAndSaveCV(app);
                    cvData = cvParsingService.getParsedCVData(app.getId());
                }
     ...
     */
    private double calculateSkillsMatch(List<String> candidateSkills, List<String> requiredSkills, List<String> preferredSkills) {
