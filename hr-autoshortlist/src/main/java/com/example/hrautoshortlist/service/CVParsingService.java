...
/**
 * Service for parsing CV files (PDF, DOCX) and extracting structured information.
...
public class CVParsingService {
     ...
     */
    public ParsedCV parseAndSaveCV(Application application) {
        logger.info("Parsing CV for application ID: {}", application.getId());

        // Check if already parsed
        if (parsedCVRepository.existsByApplicationId(application.getId())) {
            logger.info("CV already parsed for application {}", application.getId());
            return parsedCVRepository.findByApplicationId(application.getId()).orElse(null);
        }

        ParsedCV parsedCV = new ParsedCV(application);
    ...
    /**
     * Extract skills from CV text using common programming languages, frameworks, and tools
     */
    ...
    private Integer extractYearsOfExperience(String text) {
        ...
        Pattern pattern = Pattern.compile("(\\d+)\\s*\\+?\\s*(?:-\\s*\\d+\\s*)?years?\\s+(?:of\\s+)?experience", Pattern.CASE_INSENSITIVE);
    ...
    private String extractEducationLevel(String text) {
        ...
        if (lowerText.contains("ph.d") || lowerText.contains("phd") || lowerText.contains("doctorate")) {
            return "PhD";
        }
        if (lowerText.contains("master") || lowerText.contains("msc") || lowerText.contains("mba") ||
                lowerText.contains("m.sc") || lowerText.contains("m.a")) {
            return "Masters";
        }
        if (lowerText.contains("bachelor") || lowerText.contains("bsc") || lowerText.contains("b.sc") ||
                lowerText.contains("b.a") || lowerText.contains("degree")) {
            return "Bachelors";
        }
